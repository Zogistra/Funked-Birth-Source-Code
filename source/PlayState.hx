...
class PlayState extends MusicBeatState
{
	...
	var grpEstablo:FlxTypedGroup<FlxSprite>;
	var santa:BGSprite;
	var heyTimer:Float;
	...
	override public function create()
	{
		...
		{
            case 'establo':
			     var bg:BGSprite = new BGSprite('establo/Bg', -600, -200, 0.9, 0.9);
				add(bg);

			grpEstablo = new FlxTypedGroup<FlxSprite>();
			add(grpEstablo);
			var guyFiles:Array<String> = [
			'apostol',
			'Ap_N',
			'Cam_Ap',
			'maria',
			...
			for(i in 0...5)
			{
			var newGuy= new FlxSprite(guyPos[i][0], guyPos[i][1]);
			newGuy.frames = Paths.getSparrowAtlas('establo/' + guyFiles[i]);
			newGuy.animation.addByPrefix('idle', 'idle', 24, false);
			newGuy.animation.play('idle');
			grpEstablo.add(newGuy);
			}

			case 'cielo': 
				var bg:BGSprite = new BGSprite('cielo/Sky', -600, 200, 0.9, 0.9);
				add(bg);
               
			    var suel:BGSprite = new BGSprite('cielo/Suel', -500, 400, 0.9, 0.9);
				suel.setGraphicSize(Std.int(suel.width * 1.1));
				suel.updateHitbox();
				add(suel);

			case 'vsdios': 
				var bg:BGSprite = new BGSprite('vsdios/tarde', -600, 200, 0.9, 0.9);
				add(bg);
	    ...
	    if(curStage == 'vsdios')
		{
		var stageCurtains:BGSprite = new BGSprite('vsdios/Nub', 430, 420, 1.3, 1.3);
		stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
		stageCurtains.updateHitbox();
		add(stageCurtains);
		}
		...
		if (!seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'intruder-at-birth':
				 	 startVideo('cutscene');

				case 'broken-commandment':
				     startVideo('video');
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'establo':
			 for(char in grpEstablo.members)
				{
				if(curBeat % 2 == 0)
